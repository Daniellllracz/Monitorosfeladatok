...
namespace oraiBarnanenak
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Lehetőleg minden kiírást a főprogram végezzen el. Próbálj minél több kódot újrahasznosítani. Minden feladatot meg
            //kell oldani hagyományosan, és azután, ha tudod, LINQ-val is.
            //1. Hozz létre egy osztályt a monitorok adatai számára. Olvasd be a fájl tartalmát.
            List<Monitor> Monitorok = new List<Monitor>();
            StreamReader sr = new StreamReader(@"..\..\..\src\Monitorok.txt");
            sr.ReadLine();
            while (!sr.EndOfStream)
            {
                var sor = sr.ReadLine();
                Monitor monitor = new Monitor(sor);
                Monitorok.Add(monitor);
            }
            ...
            foreach (var v in Monitorok)
            {
                Console.WriteLine("Gyarto",v.Nev, "Típus", v.Tipus, "Méret:", v.Meret, "col", "Árak", v.Ar, "Ft");
            ...
            //3. Tételezzük fel, hogy mindegyik monitorból 15 db van készleten, ez a nyitókészlet. Mekkora a nyitó raktárkészlet bruttó
            //(tehát áfával növelt) értéke?
